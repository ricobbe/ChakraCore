...
public:
    ...
    bool CommitBuffer(TEmitBufferAllocation* allocation, _Out_writes_(allocation->bytesCommitted) BYTE* destBuffer, __in size_t bytes, __in_bcount(bytes) const BYTE* sourceBuffer, __in DWORD alignPad = 0);
